import { set, ref, onValue, remove, update, query, orderByChild, equalTo } from "firebase/database";
import { uid } from "uid";
...
const findOne = (url, findPattern) => {
    return new Promise(async (resolve, reject) => {
        try {
            const keyValuePairs = Object.keys(findPattern).map(key => ({ key, value: findPattern[key] }));
...
const findOneWithNotEqual = (url, findPattern) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const data = await getOrderByChildWithEqualto(url, { key, value })
            const findedData = data.length ? data.filter(x => x.id.toLowerCase() !== id.toLowerCase()) : []
            resolve(findedData)
        } catch (e) {
            reject(e);
        }
